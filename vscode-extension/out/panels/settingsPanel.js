...
class SettingsPanel {
    static open(extensionUri, context) {
        if (SettingsPanel._panel) {
            SettingsPanel._panel.reveal();
            return;
        }
        const panel = vscode.window.createWebviewPanel(SettingsPanel.viewType, "AI Copilot Settings", vscode.ViewColumn.One, { enableScripts: true });
        SettingsPanel._panel = panel;
        panel.onDidDispose(() => { SettingsPanel._panel = undefined; });
        const envPath = (0, backendManager_1.getEnvPath)(extensionUri);
        ...
        panel.webview.onDidReceiveMessage((msg) => {
            if (msg.type === "save") {
                ...
                vscode.window
                    .showInformationMessage("Settings saved. Restart backend to apply changes?", "Restart now", "Later")
                    .then((choice) => {
                    if (choice === "Restart now") {
                        (0, backendManager_1.restartBackend)(extensionUri, context);
                    }
                });
            }
        });
...
SettingsPanel.viewType = "aiCopilot.settings";
...
function writeEnv(envPath, data) {
...
function getHtml(env) {
    const val = (key, def = "") => env[key] ?? def;
    const selected = (key, match) => val("PROVIDER") === match ? "selected" : "";
    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"/>
